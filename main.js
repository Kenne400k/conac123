...
function() {
  ...
  try {
    ...
    // Bắt đầu auto require dependencies
    if (module.config.dependencies && typeof module.config.dependencies == 'object') {
      for (const reqDependencies in module.config.dependencies) {
        ...
        try {
          ...
          // Tự động cài package nếu không tìm thấy, có hỏi Y/N và lưu config
          let pkgVer = (module.config.dependencies[reqDependencies] && module.config.dependencies[reqDependencies] !== '*' && module.config.dependencies[reqDependencies] !== '') ? '@' + module.config.dependencies[reqDependencies] : '';
          logger.loader(global.getText('mirai', 'notFoundPackage', reqDependencies, module.config.name), 'warn');
          
          // Kiểm tra config bật/tắt auto install
          if (global.config.autoinstallpackage === undefined) global.config.autoinstallpackage = false;
          let allowInstall = global.config.autoinstallpackage;

          if (allowInstall === false) {
            // Hỏi người dùng
            const msg = chalk.black.bgYellow.bold("Tiến Trình đang tải package còn thiếu. Bạn có muốn tự động cài package này? (Y/N): ");
            const ans = readlineSync.question(msg);
            if (ans.trim().toLowerCase() === 'y') {
              allowInstall = true;
              global.config.autoinstallpackage = true;
            } else {
              allowInstall = false;
              global.config.autoinstallpackage = false;
            }
            // Ghi lại vào config.json
            writeFileSync(global.client.configPath, JSON.stringify(global.config, null, 4), 'utf8');
          }

          if (allowInstall) {
            console.log(chalk.black.bgYellow.bold("Tiến Trình đang tải package còn thiếu xin vui lòng đợi..."));
            try {
              execSync(`npm install ${reqDependencies}${pkgVer} --save --no-package-lock`, {
                stdio: 'inherit',
                env: process.env,
                shell: true,
                cwd: process.cwd()
              });
              // Require lại sau khi cài
              if (listPackage.hasOwnProperty(reqDependencies) || listbuiltinModules.includes(reqDependencies)) {
                global.nodemodule[reqDependencies] = require(reqDependencies);
              } else {
                global.nodemodule[reqDependencies] = require(reqDependenciesPath);
              ...
              throw global.getText('mirai', 'cantInstallPackage', reqDependencies, module.config.name, e);
            }
          } else {
            throw new Error(`Đã từ chối cài package ${reqDependencies}. Bạn có thể bật lại bằng cách chỉnh "autoinstallpackage" trong config.json.`);
          }
        }
      }
    }
  ...
  function() {
    ...
    try {
        ...
        // Tự động cài package nếu không tìm thấy
        let pkgVer = (event.config.dependencies[dependency] && event.config.dependencies[dependency] !== '*' && event.config.dependencies[dependency] !== '') ? '@' + event.config.dependencies[dependency] : '';
        logger.loader(global.getText('mirai', 'notFoundPackage', dependency, event.config.name), 'warn');
        try {
            execSync(`npm install ${dependency}${pkgVer} --save --no-package-lock`, {
                stdio: 'inherit',
                env: process.env,
                shell: true,
                cwd: process.cwd()
            ...
            throw global.getText('mirai', 'cantInstallPackage', dependency, event.config.name, e);
        }
    }
}
              }
